...
def create_asmnt_comment(assessment):
  ...
  comments = [
      assessment_integration._INITIAL_COMMENT_TMPL %
      assessment_integration._get_assessment_url(assessment)
  ]
...
def bulk_create_issuetracker_info(tracked_objs, people_emails):
  ...
  for obj in tracked_objs.values():
    try:
      ...
      # pylint: disable=protected-access
      integration_utils.normalize_issue_tracker_info(issue_info)
      ...
      issue_url = assessment_integration._ISSUE_URL_TMPL % res.get("issueId")
...
def prepare_issue_json(obj, issue_tracker_info, people_emails):
  ...
  return {
      ...
      "status": assessment_sync_job.ASSESSMENT_STATUSES_MAPPING[obj.status],
  }
